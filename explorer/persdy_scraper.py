...
class PersDYScraper(RequestWorker):
	...
	def process_to_query(self, count: int = 0, max_count: int = 1) -> bool:
		...
		if count >= max_count:
			...
			# # # 生成header, 获取首页。
			self.RCR.url = "https://www.norwegian.com/"
			...
			if self.RCR.request_to_get(is_redirect=True):
				...
				# # # 生成header, 获取首页。
				self.RCR.url = "https://www.norwegian.com/us/start/booking/avaday/?A_City=ALC&AdultCount=1&ChildCount=0&CurrencyCode=USD&D_City=LAX&D_Day=25&D_Month=202001&D_SelectedDay=25&IncludeTransit=true&InfantCount=0&R_Day=07&R_Month=202001&TripType=1"
				self.RCR.param_data = None
				self.RCR.header = self.BFR.format_to_same(self.init_header)
				self.RCR.header.update({
					"Accept-Encoding": "gzip, deflate",
